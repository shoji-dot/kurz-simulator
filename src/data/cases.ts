...
export const surgicalCases: SurgicalCase[] = ([
  ...
  },
  {
    ...
    teachingPoints: [
      ...
      '伝音再建法の分類は、ツチ骨柄の有無だけでは決まらない。重要なのは「どこからどこへ伝音を再建するか」という再建経路。本症例はツチ骨柄が欠損しており、軟骨再建面（鼓膜）からアブミ骨頭部へ直接再建するため、JOS2010ではIII型（Ⅲc相当）に分類される。',
  ...
  {
    id: 'case-011',
    // Issue-026(2026-07-29): II型→III型に表記修正。clinicalNotes記載の通りキヌタ骨長突起
    // 残存部は術中に除去されPORP頭板の支持面確保に使われるのみで再建経路には関与しない
    // (shoji確認: 長突起の残存関与は「無い」、II型という表記は臨床像の呼称としても
    // 「使っていない」)。実際の再建経路はツチ骨柄→PORP→アブミ骨頭部でありJOS2010のIIIi-M相当
    // (case-001/008と同じ論点)。
    title: '症例11: 小児慢性中耳炎 — PORP（III型 小児）',
    description: '8歳男児。3歳から反復性急性中耳炎の既往。左耳に鼓膜穿孔が持続し慢性化。術前CT・鼓室鏡にてキヌタ骨長突起の骨吸収（partial）を確認。ツチ骨柄および鼓膜臍部は温存。アブミ骨上部構造は保存されており鼓室形成III型の適応。小児のため外耳道が狭く、軟骨グラフトとPORPを組み合わせた鼓室形成術を計画。',
    difficulty: 'intermediate',
    ...
    teachingPoints: [
      '小児慢性中耳炎ではキヌタ骨長突起の骨吸収が最多パターン（III型）。ツチ骨柄は比較的保存されることが多い。',
  ...
  },
  {
    ...
    teachingPoints: [
      ...
      '伝音再建法の分類は、ツチ骨柄の有無だけで機械的に決まるものではない。重要なのは「どこからどこへ伝音を再建するか」という再建経路。本症例はツチ骨柄が欠損しており、軟骨再建面（鼓膜）からアブミ骨頭部へ直接再建するため、JOS2010ではIII型（Ⅲc相当）に分類される。',
