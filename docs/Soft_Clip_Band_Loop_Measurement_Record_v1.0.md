...
長さは実測値をEvidence A/A+として反映済み。**残る最重要未確定項目はShaft接続位置
(§1-1-A)のみ**。このファイル自体はコード変更を伴わない。
...
### 1-1-A. Shaft接続位置(優先度★★★★★・新規v1.2・現時点で残る最重要未確定項目)

写真からは、ShaftはLower Arm**根元**ではなく**途中位置**に接続しているように見えます。
これはCenterline Sweepの**開始点(起点)**をどこに置くかに直接影響するため、他の
制御点(1-1)よりも優先して確認が必要です。

確認したい内容は、**Lower Arm全長に対する**以下3点の位置関係です。

| # | 項目 | 20倍模型実測値 | 実寸換算(÷20) | 備考 |
|---|---|---|---|---|
| 5-A | Lower Arm開始点(Pocket形成部側の起点) | **(未測定)** | | Lower Armがどこから始まるか |
| 5-B | 返し曲げ終端(Lower Arm自由端、§0の「返し曲げ」形状の先端) | **(未測定)** | | 1-1の#5'と同一点でも可 |
| 5-C | Shaft中心接続位置 | **(未測定)** | | Lower Arm全長のうち、Shaftが接続する位置(根元からの距離、またはLower Arm全長に対する比率) |
