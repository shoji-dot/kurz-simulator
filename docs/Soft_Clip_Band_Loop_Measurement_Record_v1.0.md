...
コード変更を伴わない。
**Date**: 2026-07-31(v1.4更新)
**フェーズ移行(shoji確認、2026-07-31)**: Soft Clip Geometryは「設計フェーズ」から
**「実測パラメータ確定フェーズ」**へ移行。Pocket/Terminal関連は確定済み、残る
唯一の実装Blockerである Shaft接続位置(M1〜M3)の測定完了が次の実装ステップ
(Centerline Parameter Definition)の前提条件。
**v1.4での変更点(shoji明確化、2026-07-31)**: M3(Shaft中心軸との交点)の定義を
精密化。単純な「見た目の接触点」ではなく、①Shaft外径の左右端から算出したShaft
中心軸と、②Lower Armの構造中心線、の**幾何学的交点**として定義する(§1-1-A参照)。
...
| M3 | Shaft中心軸との交点 | **(未測定)** | | 下記「M3の定義(v1.4)」を参照。単純な見た目上の接触点ではない |

**M3の定義(v1.4、shoji明確化2026-07-31)**: M3は「見た目の接触点」ではなく、以下の
**2段階の幾何学的交点**として定義する。

1. Shaft外径の左右端(輪郭)から**Shaft中心軸**を算出する(円柱の中心線)。
2. その中心軸と、**Lower Armの構造中心線(Centerline)**との交点をM3とする。

つまりM3は「Shaft中心軸とArm構造中心線の幾何学的交点」であり、写真上でShaftとBand
Loopが視覚的に重なって見える位置をそのままM3とはしない。この定義により、Centerline
Sweep実装時の推測要素を減らすことができる(shoji指摘)。
